...
let _ = register_predicate "gt0" ["int"] (fun xs -> (int_of_string (List.nth xs 0)) > 0 )
let _ = register_predicate "gt1" ["int"] (fun xs -> (int_of_string (List.nth xs 0)) > 1 )
...
*)
let _ = register_predicate "invr" ["rad";"rad"] (fun xs -> 
    (int_of_string (List.nth xs 0)) = -1*(int_of_string (List.nth xs 1)))
let _ = register_predicate "invi" ["arc";"arc"] (fun xs -> 
    (int_of_string (List.nth xs 0)) = -1*(int_of_string (List.nth xs 1)))
